...
A Codex plugin that holds an output to **two eyes on the same object**: a
...
So **this plugin does not make Codex two-eyed.** Etuaptmumk — Mi'kmaw Elder Albert
...
Expose `codex/miette` through a configured Codex marketplace, install `miette` from
`/plugins`, then start a new session. Review and trust the bundled Stop hook in
`/hooks`; Codex does not run untrusted plugin hooks.

Invoke `$two-eyed-output` to write with both eyes or `$two-eyed-translator` to
rewrite a dense answer. **Hooks load at session start** — enabling mid-session requires
a new session.
...
skills/two-eyed-translator/SKILL.md    rewrites a dense output into a receivable one
```
